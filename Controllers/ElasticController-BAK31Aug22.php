...
//echo '<pre>'; print_r($params);
                    ...
                    for ($i = 1; $i <= 7; $i++)
                    {
                        ...
                        $es_data = $this->client->search($params);
                        $total_eng = $es_data["aggregations"]["total_shares"]["value"] + $es_data["aggregations"]["total_comments"]["value"] + $es_data["aggregations"]["total_likes"]["value"];
                        ...
                        $params = [
                            ...
                            'body' => [
                                'query' => [
                                    'bool' => [
                                        'must' => [
                                            [ 'query_string' => [ 'query' => $topic_query_string.' AND p_created_time:("' . $d_date . '") AND source:("Twitter" OR "Youtube" OR "Linkedin" OR "FakeNews" OR "News" OR "Pinterest" OR "Instagram" OR "Reddit" OR "Tumblr" OR "Vimeo")' ] ]
                                        ]
                                    ]
                                ]
                                ,
                    ...
                    $params = [
                        ...
                        'body' => [
                            'query' => [
                                'bool' => [
                                    'must' => [
                                        ['query_string' => ['query' => $topic_query_string . ' AND source:("Twitter" OR "Youtube" OR "Linkedin" OR "FakeNews" OR "News" OR "Pinterest" OR "Instagram" OR "Reddit" OR "Tumblr" OR "Vimeo")']],
                                        ['range' => ['p_created_time' => ['gte' => $greater_than_time, 'lte' => $less_than_time]]]
                                    ]
                                ]
                            ],
...
//print_r($params);
            ...
            else if($request["section"] == 'sources_counts' || $request["section"] == 'sources_counts_subtopic')
            {
                ...
                if($results["count"] > 0)
                {
                    $response_output .= 'Videos,'.$results["count"].'|';
                }
                ...
                if($results["count"] > 0)
                    $response_output .= 'News,'.$results["count"].'|';
                ...
                if($results["count"] > 0)
                    $response_output .= 'Twitter,'.$results["count"].'|';
                ...
                if($results["count"] > 0)
                    $response_output .= 'Pinterest,'.$results["count"].'|';
                ...
                if($results["count"] > 0)
                    $response_output .= 'Instagram,'.$results["count"].'|';
                ...
                if($results["count"] > 0)
                    $response_output .= 'Blogs,'.$results["count"].'|';
                ...
                if($results["count"] > 0)
                    $response_output .= 'Reddit,'.$results["count"].'|';
                ...
                if($results["count"] > 0)
                    $response_output .= 'Tumblr,'.$results["count"].'|';
                ...
                if($results["count"] > 0)
                    $response_output .= 'Facebook,'.$results["count"].'|';
                ...
                $results = $this->client->count($params);
                if($results["count"] > 0)
                    $response_output .= 'Web,'.$results["count"].'|';
                ...
                //['range' => ['p_created_time' => ['gte' => $greater_than_time, 'lte' => $less_than_time]]] -- Date range is not added for it.
                							
                $results = $this->client->count($params);
                
                if($results["count"] > 0)
                    $response_output .= 'GoogleMaps,'.$results["count"].'|';
                ...
                if($results["count"] > 0)
                    $response_output .= 'Tripadvisor,'.$results["count"].'|';
            ...
            else if($request["section"] == 'most_active_users')
            {
                ...
                for ($i = 0; $i < count($results["aggregations"]["group_by_user"]["buckets"]); $i ++)
                {
                    if (!empty($results["aggregations"]["group_by_user"]["buckets"][$i]["key"]))
                    {
                        ...
                        if ($user_source == 'Twitter')
                            $source_icon = '<i class="bx bxl-twitter mr-25 align-middle" title="Twitter" style="font-size: 20px; color: #00abea !important;"></i>';
                        else if ($user_source == 'Youtube')
            ...
            else if($request["section"] == 'get_posts_html_data') //This is being used to fetch json saved query for posts listing
            {
                ...
                else if($request["source_handle"] == 'Youtube')
            ...
            else if($request["section"] == 'subtopic_wordcloud' || $request["section"] == 'maintopic_wordcloud')
            {
                ...
                if(!isset($request["pdf_report_data"]) && $request["pdf_report_data"] != 'yes')
                {
                    ...
                    if($request["section"] == 'subtopic_wordcloud')
                        $chk = DB::select("SELECT wc_str, wc_str_sorted FROM wordcloud_data WHERE wc_time >= DATE_SUB(NOW(),INTERVAL 24 HOUR) AND wc_stid = ".$subtopic_session_id);
                    else if($request["section"] == 'maintopic_wordcloud')
                        $chk = DB::select("SELECT wc_str, wc_str_sorted FROM wordcloud_data WHERE wc_time >= DATE_SUB(NOW(),INTERVAL 24 HOUR) AND wc_tid = ".\Session::get('current_loaded_project'));
                    ...
                    if (!empty($results["aggregations"]["group_by_user"]["buckets"][$i]["key"]))
                    {
                        ...
                        $u_html .= '<table width="100%" cellpadding="0" cellspacing="0">
                                <tr>
                                    ...
                                    <td width="12%"><img src="https://'.$request->getHost().'/images/flags/4x3/'.$this->gen_func_obj->get_country_flag($results["aggregations"]["group_by_user"]["buckets"][$i]["grouped_results"]["hits"]["hits"][0]["_source"]["u_country"]) . '" width="35"></td>
                                    <td width="15%" style="text-align: right; font-size:1.1rem;">' . $this->gen_func_obj->format_number_data($ufollowers) . '<p style="color:#cccccc; font-size: 13px;">Followers</p></td>
            ...
            else if($request["section"] == 'get_printmedia_posts_html_data')
            {
                ...
                if (isset($request["tags"]) && !empty($request["tags"]))
                {
                    ...
                    if (!empty($topic_key_hash) && !empty($topic_urls))
                        $str_to_search = '(p_message:(' . $topic_key_hash . ' OR ' . $topic_urls . ') OR u_source:(' . $topic_urls . '))';
                ...
                if(count($results["hits"]["hits"]) > 0)
                {
                    for($i=0; $i<count($results["hits"]["hits"]); $i++)
                    {
                        ...
                        $pic = 'https://dashboard.datalytics24.com/printmedia-data-entry/assets/uploads/news_sources/'.$this->gen_func_obj->decrypt($src_data[0]->source_image, $this->gen_func_obj->get_encryption_key());

                        $emotion_icon = '&nbsp;';

                        $post_id = $results["hits"]["hits"][$i]["_source"]["p_id"];

                        if (false === file_get_contents("https://dashboard.datalytics24.com/printmedia-data-entry/assets/uploads/".$results["hits"]["hits"][$i]["_source"]["post_full_detail_doc"],0,null,0,1))
                            $detail_url = $results["hits"]["hits"][$i]["_source"]["p_url"];
                        else
                            $detail_url = 'https://dashboard.datalytics24.com/observer/pmd?pmdid='.$this->gen_func_obj->encrypt($this->gen_func_obj->encrypt($post_id, $this->gen_func_obj->get_encryption_key()), $this->gen_func_obj->get_encryption_key());
            ...
            else if($request["section"] == 'maintopic_channel_sentiments')
            {
                ...
                $sources_array = array("Videos", "News", "Twitter", "Pinterest", "Instagram", "Blogs", "Reddit", "Tumblr", "Facebook", "Web");
