...
<html>
    <head>
        ...
        <script type="text/javascript" src="{{asset('assets/survey/js/survey_form.js?v=10')}}"></script>
    ...
    ?>
    <?php if($survey->survey_id == 1298787) { //this check is implemented to make changes for survey and test, live survey code is in else case ?>
...
<?php $subtitle = 'Survey Response Form'; ?>
            ...
            <div id="site_content" style="padding-top:0px;">
                <div id="content" style="margin: 0px auto;">
                    ...
                    <?php
                    if (file_exists(getcwd() . '/surveys/survey_logos/' . $survey->survey_logo) && is_file(getcwd() . '/surveys/survey_logos/' . $survey->survey_logo)) {
                    ?>
                        <div style="width:100px; height: 100px; background-image: url('https://dashboard.datalyticx.ai/surveys/survey_logos/<?php echo $survey->survey_logo; ?>'); background-size: contain; background-repeat: no-repeat; background-position: center; margin:10px auto;"></div>
                                    <?php } ?>
                        <form id="survey_form" action="{{route('post.survey')}}" method="post">
                            ...
                            <input type="hidden" id="ask_customer_details" name="ask_customer_details" value="<?php if ($survey->survey_ask_customer_details == 'y') echo 'yes'; else echo 'no'; ?>" />
                            <div class="step-app" id="demo" style="margin-top:25px;">
                                ...
                                <?php
                                    ...
                                    $k = 1;
                                    $question_ids = '';
                                    foreach ($survey->questions as $i => $question) { $question_ids .= $question->question_id.',';
                                ?>
                                    <div class="step-tab-panel" data-step="step<?php echo $ii; ?>">
                                        <div style="border-bottom: 1px dotted; padding-bottom: 15px; text-align: center;">
                                            <h4 class="question_text" style="color:#6cafc4; font-size: 19px; padding-bottom: 12px;" id="<?php echo htmlspecialchars($question->question_id); ?>" data-question_id="<?php echo htmlspecialchars($question->question_id); ?>" data-question_type="<?php if($question->question_type == 'csat') echo 'radio'; else echo htmlspecialchars($question->question_type); ?>" data-is_required="<?php echo htmlspecialchars($question->is_required); ?>"><?php $aa = explode("|", htmlspecialchars($question->question_text));
                                    if (isset($aa[1]) && !empty($aa[1])) echo $aa[1] . '<br>' . $aa[0];  //echo htmlspecialchars($question->question_text);  ?></h4>
                                                ...
                                                <span <?php if ($question->is_rate_service) echo 'id="smileys"'; ?>>
                                                    ...
                                                    foreach ($question->choices as $j => $choice):
                                                        ...
                                                        if ($question->is_rate_service == 1 && $question->question_type == 'csat') {
                                                            ?>          
                                                            <input id="<?php echo $question_html_id; ?>" type="radio" name="question_id[<?php echo htmlspecialchars($question->question_id); ?>][]" value="<?php echo htmlspecialchars($choice->choice_text); ?>" class="<?php echo htmlspecialchars($choice->choice_text); ?>"<?php //if($k==1) echo ' checked'; ?> />
                <?php
            $k = $k+1; if($k>3) $k=1;} else if ($question->is_nps_service == 1 && $question->question_type == 'nps') {
                ?>
            ...
            <?php } elseif ($question->question_type == 'rate15') {
                ?>
                                                            <label style="display:inline-block;">
                                                                <input id="<?php echo $question_html_id; ?>" type="radio" name="question_id[<?php echo htmlspecialchars($question->question_id); ?>][]" value="<?php echo htmlspecialchars($choice->choice_text); ?>">
                                                                <span style="font-size: 0.9rem;"><?php echo htmlspecialchars($choice->choice_text); ?></span>
                                        ...
                                        ?>
                                            <div class="step-tab-panel" data-step="step<?php echo $ii; ?>">
                                                ...
                                                <div style="padding:15px 0px 25px 0px;">
                                                    <div style="text-align: center; font-size: 16px; font-weight: bold;">Personal details - تفاصيل شخصية</div>
                                                    <div id="pdetails_msg" style="color:red; display:none; text-align: center; padding: 5px;">All fields are mandatory</div>
                                                    <div style="">
                                                        <label>Name - اسم</label>
                                                        <input type="text" name="survey_customer_name" id="survey_customer_name" style="width: 100%;" required="required">
                                                    </div>
                                                    <div style="padding-top:5px;">
                                                        <label>Email - بريد إلكتروني</label>
                                                        <input type="text" name="survey_customer_email" id="survey_customer_email" style="width: 100%;" required="required">
                                                    </div>
                                                    <div style="padding-top:5px;">
                                                        <label>Phone - هاتف</label>
                                                        <input type="text" name="survey_customer_phone" id="survey_customer_phone" style="width: 100%;" required="required">
                                                    </div>
                                                </div>
                                            </div>
                                        <?php
                                ...
                                ?>
                                    <input type="hidden" name="qids" id="qids" value="<?php echo substr($question_ids, 0, -1); ?>">
                                </div>
        ...
        <!--<script type="text/javascript" src="{{asset('assets/survey/js/jquery.steps.min.js')}}"></script>-->
        <script type="text/javascript" src="{{asset('assets/survey/js/smiley-rating.js')}}"></script>
        <script type="text/javascript">
        $( document ).ready(function() {
            $('#demo').steps({
                onFinish: function () {
                    if($("#ask_customer_details").val() == 'yes')
                    {
                        if($("#survey_customer_name").val() == '' || $("#survey_customer_email").val() == '' || $("#survey_customer_phone").val() == '')
                        {
                            $("#pdetails_msg").html("All fields are mandatory");
                            $("#pdetails_msg").show();
                        }
                        else if(!isEmail($("#survey_customer_email").val()))
                        {
                            $("#pdetails_msg").html("Enter correct email address");
                            $("#pdetails_msg").show();                        
                        }
                        else
                            $("#survey_form").submit();
                ...
                onChange: function (currentIndex, newIndex, stepDirection) { 
                    ...
                    if(stepDirection == 'forward' && !validate_survey_form(currentIndex, newIndex) && currentIndex != newIndex)
                        return false;
                    else                        
                        return true;
                }
                            ...
                            <div class="step-app" id="demo" style="margin-top:25px;">
                                ...
                                ?>
                                    <div class="step-tab-panel" data-step="step<?php echo $ii; ?>">
                                        <div style="border-bottom: 1px dotted; padding-bottom: 15px; text-align: center;">
                                            <h4 class="question_text" style="color:#6cafc4; font-size: 19px; padding-bottom: 12px;" id="<?php echo htmlspecialchars($question->question_id); ?>" data-question_id="<?php echo htmlspecialchars($question->question_id); ?>" data-question_type="<?php if($question->question_type == 'csat') echo 'radio'; else echo htmlspecialchars($question->question_type); ?>" data-is_required="<?php echo htmlspecialchars($question->is_required); ?>"><?php $aa = explode("|", htmlspecialchars($question->question_text));
                                    if (isset($aa[1]) && !empty($aa[1])) echo $aa[1] . '<br>' . $aa[0]; else echo htmlspecialchars($question->question_text);  //echo htmlspecialchars($question->question_text);  ?></h4>
                                                <?php if (in_array($question->question_type, ['radio', 'checkbox', 'csat', 'nps', 'rate15'])): ?>
                                                <span <?php if ($question->is_rate_service) echo 'id="smileys"'; ?>>
                                                    <?php
                                                    foreach ($question->choices as $j => $choice):
                                        ...
                                        ?>
                                            <div class="step-tab-panel" data-step="step<?php echo $ii; ?>">
                                                ...
                                                <div style="padding:15px 0px 25px 0px;">
                                                    ...
                                                    <div style="padding-top:5px;">
                                                        <label>Phone - هاتف</label>
                                                        <input type="tel" name="survey_customer_phone" id="survey_customer_phone" style="width: 100%;" required="required" onkeypress="return isNumberKey(event);" pattern="{0-9}">
                                                    </div>
                                                </div>
                                            </div>
                                        <?php
        ...
        $( document ).ready(function() {
            $('#demo').steps({
                onFinish: function () {
                    if($("#ask_customer_details").val() == 'yes')
                    {
                        if($.trim($("#survey_customer_name").val()) == '' || $.trim($("#survey_customer_phone").val()) == '')
                        {
                            $("#pdetails_msg").html("Name and Phone is mandatory");
                            $("#pdetails_msg").show();
                        }
                        else if(!isEmail($("#survey_customer_email").val()) && $.trim($("#survey_customer_email").val()) != '')
                        {
                            $("#pdetails_msg").html("Enter correct email address");
                            $("#pdetails_msg").show();                        
                        }
                        else if($("#survey_customer_phone").val().length < 8)
                        {
                            $("#pdetails_msg").html("Phone should be at-least 8 digits");
                            $("#pdetails_msg").show();                        
                        }
                        else
                            $("#survey_form").submit();
