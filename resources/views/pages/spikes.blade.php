        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h4 class="card-title">List of spike alerts</h4>
            </div>
                ...
                <table class="table">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        @if($filter->spike)
                        <span span class="badge badge-info">{{ $filter->spike->total_mentions }}</span>
                        @endif
                        {{-- @if($filter->filter)
                        ...
                        @endif
                        @if($filter->filter)
                          @foreach (explode(',',$filter->spike->filter_emails) as $keyword)
                            <span span class="badge badge-info">{{ $keyword }}</span>
                          @endforeach
                        @endif --}}
                      </td>
                      <td>
                        @if (@empty($filter->spike->filter_freq))
                          Not set
                        @else
                          {{ $filter->spike->filter_freq }}
                        @endif
                      </td>
                      <td>
                        <a href="{{ route('spike.detail',$filter->topic_id) }}" class="btn mr-1 mb-1 btn-success btn-sm">Manage</a>
                        <button type="button" class="btn mr-1 mb-1 btn-danger btn-sm deleteAlert" @if($filter->spike) id="{{ $filter->spike->id }}" @else   @endif>Delete</button>
                      </td>
                    </tr>
                    @endforeach
                  </tbody>
